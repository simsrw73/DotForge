...
#            ffd picker (fd), frg picker (ripgrep), fcd smart-cd (zoxide)
...
# ── Group 4: PS modules ───────────────────────────────────────────────────────
# Registers fzf key handlers (PSFzf), file icons (Terminal-Icons), prompt (oh-my-posh)
# These check Get-Module -ListAvailable — silently skip if not installed
Register-DFTool -Name PSFzf, Terminal-Icons, oh-my-posh

# ── Completions: refresh after weekly tool updates ───────────────────────────
# Uncomment to force-refresh all dynamic completers (rg, chezmoi, gh, etc.)
