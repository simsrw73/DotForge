...
- **Package-universe Phase D — categorization engine + hybrid classifier** — `build/Build-DFPackageUniverseCategories.ps1` classifies every merged tool into a closed taxonomy (coarse `domain` + `function`/`worksWith` facets + `interface`, plus `alternativeTo`) by reading each tool's own docs (any-host README → doc page → metadata). Each result is cached durably (keyed on a stable identity signal, not the volatile tool_id) and exported to a version-controlled `data/package-universe-classifications.jsonc`, so the run is resumable, budget-capped, and survives DB rebuilds and re-clustering. Bulk classification uses a small OpenAI model, with `New-DFPackageUniverseClaudeClassifySeam` (Anthropic Messages API, forced tool-use) as the default escalation seam for low-confidence/`nothing_fits` results — live spot-checks found the cheap model confidently mistagging unrelated tools (e.g. an IRC client, an LDAP browser) as `vcs-client`, which Claude Haiku 4.5 corrected while leaving true positives untouched; falls back to same-model escalation with a warning if `ANTHROPIC_API_KEY` is absent. Both provider seams share the exact system prompt, JSON schema, and user payload (`Get-DFPackageUniverseClassifierSystemPrompt`, `New-DFPackageUniverseClassifierSchema`, `Get-DFPackageUniverseClassifierUserPayload`) so they can never classify against different taxonomies. Build-only; no public module surface change.
- **Package-universe Phase D — rate-limit resilience (reactive + proactive)** — both classifier seams recognize a rate-limit or credit-exhaustion error from either provider (`ConvertTo-DFPackageUniverseRateLimitSignal`) and stop the batch immediately via a `DF_RATE_LIMITED::<resetAt>::<message>` marker, instead of deferring every remaining tool up to `BudgetCalls` one failure at a time (fixed a real incident that generated 734 pointless deferred rows). They also capture the provider's real-time remaining-requests count on every *successful* call (`x-ratelimit-remaining-requests` for OpenAI, `anthropic-ratelimit-requests-remaining` for Anthropic), so `Invoke-DFPackageUniverseCategorizeRun`'s new `-RateLimitSafetyMargin` (default 20) can stop a run proactively before ever hitting the wall. The reconciliation summary reports `RateLimited`/`RateLimitResetAt`; the orchestrator prints a clear "stopped early, resets around `<time>`" warning.
- **Package-universe Phase D — vocab-gap review tooling + semantic clustering** — `build/Invoke-DFPackageUniverseVocabReview.ps1` and `docs/package-universe-review-guide.md` document and drive the resolution mechanism for all three of the pipeline's review queues (Phase B identity-clustering curation, Phase C's informational-only license-conflict flag, Phase D categorization). Applying it at scale showed the `nothing_fits` backlog's exact-string aggregation badly undercounted real signal — a tool with no vocabulary fit typically suggests 3+ near-synonym phrasings of one concept per event (76% of 8,215 distinct raw strings were exact-match singletons purely from wording variance). A three-pass semantic clustering (full trace preserved at `data/package-universe-vocab-clustering-history.jsonc`) folded this into 93 new categories meeting a ≥3-tool discoverability bar plus 20 corrections that were unrecognized synonyms of existing vocabulary; `build/Invoke-DFPackageUniverseVocabClusteringApply.ps1` applies the result end to end (vocab files, cache clearing via the new `Remove-DFPackageUniverseVocabGapClassificationsBulk` primitive, durable decision log, and re-export — all as one idempotent, re-runnable step). Applying this surfaced and fixed three real bugs along the way, each now covered by a regression test: `Build-DFPackageUniverseCategories.ps1` reads vocab from the *generated* `data/tool-categories.json`, not the `build/categories/taxonomy.jsonc` source the apply script edits, so `Build-DFCategoryDb.ps1` must be re-run after any vocab edit; `Build-DFCategoryDb.ps1` itself mis-globbed `build/categories/domains.jsonc` (Phase D's coarse-domain file) as a tool-fragment; and the apply script must re-export `classifications.jsonc` as its own last step, since `Build-DFPackageUniverseCategories.ps1` unconditionally re-imports that file at the start of every run and would otherwise silently restore just-cleared rows.
- **Package-universe Phase D — live preview through trifle** — `build/Export-DFPackageUniversePreviewCategoryDb.ps1` writes the *live* package-universe classifications into `Get-DFCategoryDb`'s existing refreshed-copy slot (`$Env:XDG_DATA_HOME/dotforge/tool-categories.json`), so `Get-DFCategoryList`/`trifle -Category`/`-WorksWith` browse the real 25k-tool taxonomy without touching the shipped `data/tool-categories.json` or waiting on Plan 2's quality-gated promotion. Only classifiable tools are included (valid `interface`, non-empty `function`); ~1,500 colliding tool names across the universe (e.g. "signal", "git" as genuinely different real-world tools) are disambiguated with a `(tool_id)` suffix rather than silently overwriting each other. Building this found and fixed a real, latent test-isolation gap: `Get-DFCategoryDb -Path <fixture>` never fully isolates from `$Env:XDG_DATA_HOME` by design (only substitutes for the shipped side of its refresh-precedence check), so `tests/Get-DFCategoryDb.Tests.ps1` and `tests/Get-DFCategoryList.Tests.ps1` silently assumed no real refreshed file ever existed on the dev machine — an assumption this feature's own preview file immediately broke. Both files now isolate `$Env:XDG_DATA_HOME` for their duration. Also documented a real scale limitation found along the way: `Find-DFPackage -Category`/`-WorksWith` resolves every match through a sequential per-tool catalog query, fine for the ~78-tool shipped seed but slow (minutes) at full-universe scale — tracked in `TODO.md`, not fixed here.
