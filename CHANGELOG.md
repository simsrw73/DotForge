...
- **Package-universe Phase D — Claude escalation seam** — `New-DFPackageUniverseClaudeClassifySeam` (Anthropic Messages API, forced tool-use for structured output) is now the default `-Escalate` seam in `Build-DFPackageUniverseCategories.ps1`, replacing Plan 1's same-model escalation. Live-batch spot-checks found the cheap model confidently mistagging unrelated tools (an IRC client, a crypto wallet, an LDAP browser, an HDL simulator) as `vcs-client` at high stated confidence, which never reached the old low-confidence-only escalation trigger; Claude Haiku 4.5 corrected all of these in a live 6-case regression check while leaving true positives (`git-cola`, `lazygit`) untouched. Falls back to same-model escalation with a warning when `ANTHROPIC_API_KEY` is absent, so a run never hard-fails for lacking it. The system prompt, JSON schema, and user payload are now shared verbatim between both provider seams (`Get-DFPackageUniverseClassifierSystemPrompt`, `New-DFPackageUniverseClassifierSchema`, `Get-DFPackageUniverseClassifierUserPayload`) so the two providers cannot silently classify against different taxonomies.
- **Package-universe Phase D — rate-limit/quota-exhaustion detection** — both classifier seams now recognize a rate-limit or credit-exhaustion error from either provider (`ConvertTo-DFPackageUniverseRateLimitSignal`) and signal it distinctly via a `DF_RATE_LIMITED::<resetAt>::<message>` marker; `Invoke-DFPackageUniverseCategorizeRun` stops the batch immediately on that signal instead of deferring every remaining tool up to `BudgetCalls`, and the reconciliation summary gains `RateLimited`/`RateLimitResetAt` (parsed from OpenAI's "try again in `<duration>`" text via `ConvertFrom-DFPackageUniverseRateLimitDuration`, when available). Fixes a real incident where an OpenAI daily-request-cap exhaustion mid-run generated 734 pointless deferred rows instead of stopping cleanly; the orchestrator now prints a clear "stopped early, resets around `<time>`" warning.
- **Package-universe Phase D — proactive rate-limit throttling** — both classifier seams now capture the provider's real-time remaining-requests count on every *successful* response too (`x-ratelimit-remaining-requests`/`x-ratelimit-reset-requests` for OpenAI, `anthropic-ratelimit-requests-remaining`/`-reset` for Anthropic -- verified against live responses, not assumed from docs), returned as `RateLimitRemaining`/`RateLimitResetAt`. `Invoke-DFPackageUniverseCategorizeRun` gains `-RateLimitSafetyMargin` (default 20): once remaining drops below it, the run stops immediately after saving the in-flight tool's good result, instead of waiting to actually hit the wall. Complements the reactive `DF_RATE_LIMITED` detection above -- this catches the daily cap *before* a failure, that catches it if something still slips through.
- **Package-universe Phase D — vocab-gap review helper** — `build/Invoke-DFPackageUniverseVocabReview.ps1` and `docs/package-universe-review-guide.md`. The `nothing_fits` review queue (3,045 rows) is really only ~30-50 distinct recurring `suggested_terms` once aggregated by frequency; the helper walks a reviewer through that aggregated list (a plain numbered menu, no fzf/module dependency), and on promote surgically inserts the term into `build/categories/domains.jsonc`/`taxonomy.jsonc` (comment-preserving, `Add-DFPackageUniverseVocabValue`), clears every cached classification the term affects so those tools re-classify on the next Phase D run, and records the decision in `data/package-universe-vocab-decisions.jsonc` so it never resurfaces. The review guide documents the resolution mechanism for all three of the pipeline's review queues (Phase B identity-clustering curation, Phase C's informational-only license-conflict flag, Phase D categorization) end to end for the first time.
- **Package-universe Phase D — vocab-gap synonym clustering + bulk apply** — after the universe finished classifying, the `nothing_fits` backlog turned out to have exact-string aggregation badly undercounting real signal: a tool with no vocabulary fit typically suggests 3+ near-synonym phrasings of one concept in a single event (avg 3.12 terms/tool across 5,472 tools; 76% of the 8,215 distinct raw strings were exact-match singletons purely from wording variance, not genuine one-off categories). A three-pass semantic clustering (documented in full at `data/package-universe-vocab-clustering-history.jsonc`, tracing every raw suggested term through to its final category) folded this down to 93 new categories meeting a ≥3-tool discoverability bar, plus 20 corrections that were actually unrecognized synonyms of already-existing vocabulary values. `build/Invoke-DFPackageUniverseVocabClusteringApply.ps1` applied the result: 93 categories added (128 `function` + 26 `worksWith`, 0 new `domain` values), 4,983 cached classifications cleared for re-run (89% of the backlog), 8,064 decisions recorded. New reusable primitive: `Remove-DFPackageUniverseVocabGapClassificationsBulk` (clears rows matching any of several terms in one table scan, vs. one scan per term).
- **Package-universe Phase D — fix: derived `tool-categories.json` staleness + `domains.jsonc`/`Build-DFCategoryDb.ps1` collision** — the 93 categories added above never actually reached the live classifier: `Build-DFPackageUniverseCategories.ps1` reads vocab from `data/tool-categories.json` (a *generated* file), not the `build/categories/taxonomy.jsonc` source the apply script edited, and nobody had regenerated it. Running `Build-DFCategoryDb.ps1` to fix that surfaced a second, independent, pre-existing bug: it globs every `*.jsonc` in `build/categories/` and treats every non-`taxonomy.jsonc` file as a tool-fragment, so `domains.jsonc`'s top-level `domain`/`schemaVersion` keys were misread as literal tool names and failed schema validation -- a landmine nobody had tripped since `domains.jsonc` predates the script's next run. Fixed by excluding `domains.jsonc` from the tool-fragment walk (regression test added); `data/tool-categories.json` regenerated (function 36 → 128, worksWith 26).
- **Package-universe Phase D — fix: apply script must re-export classifications.jsonc** — a third bug in the same saga: `Build-DFPackageUniverseCategories.ps1` unconditionally re-imports the committed `classifications.jsonc` at the start of every run, so directly clearing rows in the live DB (as the vocab clustering apply script does) without re-exporting first meant the very next Phase D run's own opening import step silently restored the stale, just-cleared rows before its classify loop ever ran. `Invoke-DFPackageUniverseVocabClusteringApply.ps1` now re-exports `data/package-universe-classifications.jsonc` as its own last step (load-bearing, not cosmetic) so this class of mistake can't recur; added the regression test the script should have had from the start.
