...
- [ ] **Port `Initialize-DFCompletionStack.Tests.ps1` to Pester 6** — the file uses `Assert-MockCalled` (11×), removed in Pester 6.0.1. With Pester 6 installed alongside 5.8.0, `Invoke-Pester` defaults to 6 and this one file fails ("`Assert-MockCalled` may only be used inside a Describe block"); the rest of the suite passes under both. Port its `Assert-MockCalled` calls to `Should -Invoke` so the suite is green under Pester 5.8 **and** 6.0.1. Until then, pin runs: `Import-Module Pester -RequiredVersion 5.8.0 -Force`. (Surfaced 2026-07-24 during the path-normalization work, when a Pester 6 install appeared mid-session.)
